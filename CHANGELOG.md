...
### Added
- **Per-source TCP/DoT connection caps.** TCPServer and DoTServer now track connections per client IP. A source exceeding `server.max_tcp_conns_per_client` or `server.max_dot_conns_per_client` (both default 16) has new connections silently closed — slow-loris defence beyond the global connection pool. [server/tcp.go](server/tcp.go), [server/dot.go](server/dot.go).
- **Per-delegation NS name cap.** `extractDelegation` now accepts a `maxNames` parameter (config key `resolver.max_ns_names_per_delegation`, default 13). NS hostnames beyond the cap are dropped from the delegation — bounds NXNS amplification (CVE-2020-8616) beyond the existing per-request query budget. [resolver/delegation.go](resolver/delegation.go).
- **NSEC3 hash-computation budget and salt-length cap.** Per-response on-wire NSEC3 hash budget (600 units), per-lookup aggressive-cache NSEC3 hash budget (150 units), and a 128-byte salt-length ceiling — closes the CVE-2023-50868 hash-DoS vector beyond the existing per-record iteration cap and records-per-proof cap. Budgets follow the same variadic-threading pattern as the existing cryptoBudget so direct callers (tests) are unaffected. [dnssec/nsec3.go](dnssec/nsec3.go), [dnssec/validator.go](dnssec/validator.go), [cache/nsec3_aggressive.go](cache/nsec3_aggressive.go).
- **Compliance matrix entries for hardening budgets.** Three new UI compliance rows for NS-Name Limit, Per-Source TCP/DoT Cap, and NSEC3 Hash Budget + Salt Cap. [web/ui/src/data/rfcCompliance.ts](web/ui/src/data/rfcCompliance.ts), [docs/rfc-compliance-matrix.md](docs/rfc-compliance-matrix.md).

## [0.8.32] - 2026-07-16

### Fixed
- **DNSSEC false-Bogus on signed NSEC zones with a mixed-case NSEC Next Domain Name (RFC 6840 §5.1).** `canonicalRData` downcased the Next Domain Name in NSEC RDATA (and the signer name in RRSIG RDATA) when building the canonical form for signature verification. RFC 4034 §6.2 erroneously listed NSEC and RRSIG among the downcased types; RFC 6840 §5.1 corrects this — those RDATA names are used in original case. Because signers emit the NSEC Next Domain Name in the original case of the next owner name, downcasing it corrupts the reconstructed canonical RRset and the RRSIG over the NSEC fails to verify, yielding **SERVFAIL + EDE 6 (DNSSEC Bogus)** for the NODATA/NXDOMAIN denial. This hits any signed NSEC zone whose chain contains a mixed-case owner label — the ubiquitous `_DMARC` TXT-policy record is the common trigger; observed live against `fedoraproject.org SRV`/`HTTPS` (apex NSEC → `_DMARC.fedoraproject.org.`). `canonicalRData` no longer downcases NSEC/RRSIG RDATA. Regression pins: [dnssec/rfc6840_nsec_case_test.go](dnssec/rfc6840_nsec_case_test.go) (sign an NSEC with a mixed-case next-domain independently of the validator's canonicalisation; a downcasing validator fails to verify), plus updated [dnssec/rfc4034_canonical_rdata_test.go](dnssec/rfc4034_canonical_rdata_test.go) and [dnssec/verify_test.go](dnssec/verify_test.go). [dnssec/verify.go](dnssec/verify.go).
- **Cached negative responses (NXDOMAIN/NODATA) lost their AD bit on re-serve.** `StoreNegative` recorded no DNSSEC verdict, so a validated (Secure) denial came back from cache with `DNSSECStatus=""` and `buildCacheResponse` cleared AD on the second and later hits — every cached signed denial silently downgraded to AD=0, diverging from mainstream validators (a differential against 1.1.1.1 over warmed queries flagged it on both NSEC and NSEC3 zones). New `StoreNegativeWithStatus` records the verdict; the resolver's NXDOMAIN/NODATA paths and the handler's negative-cache writes now pass it through, so cached secure denials keep AD while unsigned/insecure denials stay AD=0. Regression pin: [cache/rfc4035_negative_status_test.go](cache/rfc4035_negative_status_test.go). [cache/cache.go](cache/cache.go), [resolver/resolver.go](resolver/resolver.go), [server/handler.go](server/handler.go). After both fixes the resolver agrees with 1.1.1.1 on 165/165 rcode+AD pairs (20 zones × 11 qtypes, warmed).
- **DS query for an unsigned (opt-out) delegation returned SERVFAIL instead of NOERROR/Insecure.** A direct client `DS` query for the millions of unsigned zones delegated under `.com` / `.net` (e.g. `google.com`, `amazon.com`, `microsoft.com`, `facebook.com`, `wikipedia.org`) is answered with NODATA proven by an **opt-out** NSEC3 span. The generic NSEC3 NODATA proof (`VerifyNSEC3Denial5155`) requires an NSEC3 that *matches* the qname hash with the DS bit clear; an opt-out delegation has no such record (the name sits inside an opt-out span), so the proof came up inconclusive and `validateDenialResponse` fell through to **Bogus → SERVFAIL**. Unbound, BIND, and the public 1.1.1.1 / 8.8.8.8 validators all return NOERROR with AD=0 (Insecure) here. `validateDenialResponse` now applies a DS-specific fallback for `TypeDS`: when the already-authenticated NSEC3 set proves DS absence via the RFC 5155 §7.2.4 opt-out (or matching-DS-clear) form — the exact proof the trust-chain walker trusts to declare an insecure delegation — the verdict is **Insecure**, not Bogus. Scoped to `TypeDS` and to authenticated NSEC3 only, so it cannot manufacture a denial; a covering NSEC3 *without* the opt-out flag still stays Bogus (the empty-DS spoofing boundary). Regression pin (opt-out→Insecure AND non-opt-out→Bogus): [dnssec/rfc5155_ds_optout_insecure_test.go](dnssec/rfc5155_ds_optout_insecure_test.go). [dnssec/validator.go](dnssec/validator.go). Verified live: `google.com/amazon.com/microsoft.com/facebook.com/wikipedia.org DS` now NOERROR/AD=0, `com/org/verisign.com/cloudflare.com DS` still NOERROR/AD=1, `dnssec-failed.org` still SERVFAIL.
- **AD bit dropped from direct DNSKEY / DS / NS answers served from an internally-seeded cache entry.** A direct client query for a signed zone's `DNSKEY`, a signed delegation's `DS`, or a signed zone's apex `NS` validated correctly on a cold cache but could return AD=0 (and, for NS, without the RRSIG) once the shared answer cache held a resolver-internal, unvalidated entry for that name/type: `resolver.QueryDNSSEC` (validator-driven, validation skipped) stores DNSKEY/DS RRsets fetched for a trust-chain walk with an empty DNSSEC status, and `cacheDelegation` stores the **parent-side delegation NS** from a referral under `(zone, NS)` — which, per RFC 2181 §5.4.1, is less trustworthy than the child's own authoritative apex NS and carries no RRSIG. A later direct client query was served that unvalidated entry. A differential comparison against a public validating resolver (1.1.1.1) over 20 signed/unsigned zones × 7 qtypes flagged the apex-NS case on every signed zone; after the fix the two agree on all 140 rcode+AD pairs. The handler now treats a `DNSKEY`/`DS`/`NS` cache hit carrying no DNSSEC status as a miss (only when a validator is active), forcing a full validating resolution that fetches the signed RRset from the child and re-caches it *with* its status. Scoped to those three meta-types and the empty status only, so every normal cached answer is untouched; a non-secure entry never SERVFAILs (it revalidates). Regression pin: [server/rfc4035_meta_type_revalidation_test.go](server/rfc4035_meta_type_revalidation_test.go). [server/handler.go](server/handler.go). Verified live: `cloudflare.com/isc.org/ietf.org/… NS`, `ietf.org DNSKEY`, and `org/com/verisign.com DS` now return AD=1 even after unrelated queries warm the cache, while unsigned zones stay AD=0.
- **DNSSEC false-Bogus on signed multi-record RRsets (wrong RFC 4034 §6.3 canonical ordering).** `canonicalRRSetWire` sorted the RRs of an RRset by their full canonical RR wire form (owner + type + class + original-TTL + **RDLENGTH** + RDATA) instead of by the RDATA alone. Owner/type/class/TTL are constant across an RRset so they were harmless, but the 2-byte RDLENGTH is **not** part of the RDATA — prefixing it made every shorter RDATA sort ahead of a longer one regardless of content, so any RRset containing a shorter-but-lexicographically-greater RDATA was reconstructed in a different order than the signer used. The signature hash then mismatched and the resolver returned **SERVFAIL + EDE 6 (DNSSEC Bogus)** for perfectly valid signed data. Observed in the wild against `cloudflare.com CAA`, `cloudflare.com MX`, and any comparable variable-length signed RRset (CAA/MX/TXT/SRV/…); positive answers, denial proofs, and genuinely-bogus zones (`dnssec-failed.org`) were unaffected, which is why it evaded the existing suite. The sort now keys on the canonical RDATA only (`bytes.Compare` already gives the RFC 4034 §6.3 "absence of an octet sorts before a zero octet" prefix semantics). Regression pin in [dnssec/rfc4034_rrset_order_test.go](dnssec/rfc4034_rrset_order_test.go): an RRset is signed in the independently-computed correct order and must verify — the test fails under the RDLENGTH-prefixed sort and passes under the fix. Verified end-to-end that `cloudflare.com CAA/MX`, `isc.org MX/CAA`, `ietf.org MX/CAA`, and 15+ other signed RRsets now return NOERROR + AD while `dnssec-failed.org`, `sigfail.verteiltesysteme.net` still SERVFAIL. [dnssec/verify.go](dnssec/verify.go).
- **Malformed responses (two OPT records) on every cold EDNS answer.** A fresh recursive resolution copies the upstream authoritative server's additional section through `ResolveResult.Additional`, which carries that server's own OPT pseudo-record. `buildResponse` then appended the resolver's own OPT for the client without removing the relayed one, so every cold positive answer to an EDNS client shipped **two OPT records** — a violation of RFC 6891 §6.1.1 ("a DNS message MUST NOT contain more than one OPT RR") that `dig` flags as "; Warning: Message parser reports malformed message packet." The cache path was clean (stored entries never include OPT), so only the every-cold-query path was affected. `buildResponse` now strips any inbound OPT from all sections before generating exactly one server OPT; this also prevents a relayed OPT from leaking EDNS to a non-EDNS client. Regression pins in [server/rfc6891_single_opt_test.go](server/rfc6891_single_opt_test.go). [server/handler.go](server/handler.go).
...
### Hardened
Global work-budget backstops, closing the highest-severity gaps from a cross-resolver audit against Unbound / BIND 9 / Knot Resolver / PowerDNS Recursor. The common theme: the resolver already bounded work *per RRset* and *per recursion depth*, but lacked the *per-whole-request* and *per-whole-response* budgets that the 2020 (NXNS) and 2024 (KeyTrap) advisories forced every competitor to add.

- **Global per-request outbound-query budget + wall-clock deadline (NXNS / runaway-referral backstop).** `MaxDepth` bounds recursion *depth* but nothing bounded the *total* number of upstream queries one client request could trigger across all its CNAME hops, QNAME-minimisation steps, and nameserver-address sub-resolutions — the exact amplification surface of NXNSAttack (the CVE-2020-8616 / -12662 / -12667 / -10995 family), where a delegation pointing at many glueless NS names fans one inbound query into a flood. NS targets were already resolved *sequentially* with first-success short-circuit (keeping amplification far below the parallel-fan-out resolvers), but a crafted nested-delegation tree could still issue unbounded sub-queries within the depth limit. New: a per-request `reqBudget` (shared across the whole resolution tree via the loop-detection `visitedSet`, including NS-address sub-resolutions which previously started with a fresh, unbounded state) charges every outbound query and collapses the request to SERVFAIL once the budget is exhausted. Two new clamped config knobs: `resolver.max_queries_per_request` (default **200** — generous headroom over the deepest legitimate CDN/CNAME chains, which issue low hundreds; competitors: Unbound `max-sent-count` 32 / `MAX_GLOBAL_QUOTA` 200, BIND `max-recursion-queries` 32, PowerDNS `max-qperq` 50) and `resolver.request_timeout` (default **20s** wall-clock deadline; Knot 10s, PowerDNS 7s). Both are floored to their defaults by `clampConfigBounds` if set ≤ 0 (a 0 budget would SERVFAIL the first query of every request; a 0 deadline would time out before the first query — the same inverted-zero black-hole shape as the existing `upstream_timeout` / `max_cname_depth` floors). Pins in [config/clamp_request_budget_test.go](config/clamp_request_budget_test.go) and [resolver/req_budget_test.go](resolver/req_budget_test.go); verified end-to-end that the real 5-hop `eu-central-1.lightsail.aws.amazon.com` chain resolves untouched under the default budget while a `max_queries_per_request: 2` instance SERVFAILs an uncached recursion.
- **Global per-response DNSSEC crypto budget (completes the KeyTrap mitigation).** `maxRRSIGVerifyAttempts = 16` capped expensive signature verifications **per RRset**, but KeyTrap (CVE-2023-50387) showed that a per-RRset cap *leaks*: an attacker can spread the cost across the multiple RRsets and proofs validated for one answer. Worse, the two **denial / insecure-delegation** verification loops (`validateDenialResponse`, `verifyDSDenial`) iterated attacker-controlled authority-section RRSIGs with **no cap at all** — a crafted NXDOMAIN/NODATA/denial-of-DS response with many RRSIGs (plus colliding key tags) forced unbounded `VerifyRRSIG` calls. New: a per-response `cryptoBudget` (cap **`maxCryptoVerifyPerResponse = 32`**) is created once per top-level `ValidateResponse*` call and threaded — variadically, so existing direct callers and tests stay unlimited — to all three `VerifyRRSIG` sites (answer RRset, trust chain via `verifyDSDenial`, and the NSEC/NSEC3 denial loop), so the *total* asymmetric verifications across the whole validation of one response are bounded regardless of how the attacker distributes them. 32 covers the most elaborate legitimate validation (answer + DNSKEY chain + denial across a couple of zones mid algorithm-rollover) while bounding crypto amplification to a small constant. Matches PowerDNS `max-signature-validations-per-query=30` and Unbound's 16 global suspensions. Pin in [dnssec/crypto_budget_test.go](dnssec/crypto_budget_test.go); the full existing DNSSEC suite (trust chains, denials, rollovers) passes unchanged under the cap, confirming no false-positive regression.
- **NSEC3 records-per-proof cap (`MaxNSEC3RecordsPerProof = 16`).** A legitimate NXDOMAIN/NODATA denial proof carries only a handful of NSEC3 records (closest-encloser + next-closer + wildcard, plus extras for opt-out spans); a flood of hundreds is an NSEC3 hash-DoS attempt (the CVE-2023-50868 family). The denial validator now refuses a proof carrying more than 16 NSEC3 records up front, before any closest-encloser hashing runs (Knot refuses > 8, PowerDNS caps at 10; 16 leaves comfortable opt-out headroom). The per-record iteration cap stays at 100 (RFC 9276 interop ceiling — lowering it to a competitor-style 50 would risk marking legitimate 51–100-iteration zones insecure), and the new per-response crypto budget already bounds the *verify* work on the proof. [dnssec/nsec3.go](dnssec/nsec3.go).

Audit note: the SAD-DNS side channel (CVE-2020-25705 / -2021-20322) is already mitigated by connected UDP sockets plus the full anti-spoofing stack (0x20, DNS cookies, TXID + source-port entropy, DNSSEC); the one remaining hardening — `IP_PMTUDISC_OMIT` on the upstream UDP socket — is a Linux-only socket option deferred to a follow-up so it can be tested on the target platform. The full gap analysis lives in [docs/resolver-hardening-gap-analysis-2026-06.md](docs/resolver-hardening-gap-analysis-2026-06.md).

## [0.8.28] - 2026-06-25

### Fixed
- **Deep CNAME chains were unresolvable — DNS 0x20 case randomization defeated response name compression.** Real-world trigger: `eu-central-1.lightsail.aws.amazon.com`, a 5-hop cross-zone CNAME chain ending at an AWS Global Accelerator A record. A client (`nslookup`, systemd-resolved, anything) got `;; Truncated, retrying in TCP → No answer`. Root cause: with `resolver.caps_for_id` (RFC 5452 §9.2 "0x20") enabled — the default since v0.6.13 — each hop of the chain is resolved with independent random casing, so the same domain comes back from upstream as `amazon.com` at one owner and `AMAZoN.CoM` at the next. `dns.EncodeName` keyed its compression dictionary on the *exact-case* name ([dns/name.go:103](dns/name.go)), so these variants never matched and the entire chain was written out uncompressed — inflating the response from 463 B (compressible) to 734 B, past the 512-byte UDP limit, forcing truncation. The fix keys the dictionary on `strings.ToLower(name)` (RFC 1035 §2.3.3 / §3.1 — names are case-insensitive and a compression pointer may target an earlier occurrence regardless of case); the bytes written stay in their original case, only the lookup is folded. The chain now packs to 463 B and resolves in a single UDP packet with 0x20 still fully active (the anti-spoofing randomization is on the *upstream* path; only the client-facing wire is compressed). Pin in [dns/compression_caseinsensitive_test.go](dns/compression_caseinsensitive_test.go).
- **TCP / DoT / DoH responses were truncated like UDP — the "retry over TCP" fallback returned an empty answer.** `MainHandler.Handle` is shared by the UDP, TCP, and DoT listeners and the DoH endpoint, but it called `maybeTruncateUDP` unconditionally with no notion of transport ([server/handler.go](server/handler.go)). So a response over 512 bytes set TC=1 and zeroed the answer section even on a stream transport — where RFC 7766 §5 removes the size limit and there is no further TCP to retry on. Combined with the compression bug above, this made the CNAME chain a hard failure: UDP truncated (correctly) → client retried over TCP → TCP *also* returned a header-only TC=1 stub → "No answer". Fix: `Handle` now derives `isStream` from `clientAddr.Network()` (tcp* = stream; nil/udp = datagram) and threads it through `buildResponse` / `buildCacheResponse` / `buildCacheResponseECS` / `buildMinimalANYResponse` into `maybeTruncateUDP`, which returns the full message untouched on stream transports. UDP datagram clients still truncate-and-retry exactly as before. Pins in [server/stream_no_truncate_test.go](server/stream_no_truncate_test.go).
- **Responses larger than 4 KiB were silently dropped on every transport.** Outbound client responses were packed into a fixed 4096-byte buffer (`packToOwnedBytes` via the pooled buffer, and `appendECSToResponse`'s literal `make([]byte, 4096)`). `dns.Pack` returns `errBufferFull` on overflow with no growth, so any response exceeding 4 KiB — a long CNAME chain, a large RRset, a DNSSEC answer with multiple RRSIGs — failed to pack: `buildResponse` returned an error, `Handle` returned `(nil, err)`, and the TCP/DoT loops closed the connection with no answer (UDP dropped it instead of truncating). This is precisely the path the TCP-truncation fix above now routes large answers onto. Fix: both pack sites retry once at the 64 KiB DNS-over-TCP message ceiling (RFC 7766 §8) when the 4 KiB scratch buffer overflows, so the answer is delivered intact on a stream transport (and still truncated afterwards for UDP). Pin: [server/coverage_test.go](server/coverage_test.go) `TestBuildResponsePackError` (rewritten from asserting the old drop-on-overflow behaviour to asserting intact delivery + UDP truncation).
- **Domain names in RRSIG / NSEC / DNAME / SRV RDATA were compressed, violating RFC 3597 §4.** `packRData` re-encoded the RRSIG Signer's Name, NSEC Next Domain Name, DNAME target, and SRV target through the compression-aware `EncodeName` ([dns/wire.go](dns/wire.go)). RFC 3597 §4 forbids a receiver from decompressing names inside the RDATA of types not on the RFC 1035 §4.1.4 well-known list, so a DO=1 downstream validator (systemd-resolved, an unbound stub) reads an emitted `0xC0…` pointer as literal name bytes and fails DNSSEC validation; RFC 4034 §3.1.7 / §4.1.3, RFC 6672 §2.1 and RFC 2782 each mandate these names be uncompressed. The defect was latent but became materially more likely once compression keying went case-insensitive (above), because the signer/next/target name now matches an earlier owner far more often. Fix: these four names are written with a new `writeUncompressedName` helper (full labels, no dictionary side effects); NS/CNAME/PTR/MX/SOA stay compressed as RFC 1035 permits. Pin: [dns/rdata_no_compression_test.go](dns/rdata_no_compression_test.go).
- **RRL slip sent a TC=1 "retry over TCP" stub to clients already on TCP / DoT / DoH.** The Response Rate Limiter's slip action ([server/handler.go](server/handler.go), both the cache-hit and fresh-resolution sites) called `buildSlipResponse` — which sets TC=1 to force a UDP retry over TCP — regardless of transport. For a stream client that retry is impossible (it is already on TCP) and RFC 7766 §4 says TC SHOULD NOT be set on a TCP response, so the slip became a dead end: an empty, answerless reply the client cannot recover from. Slip's whole purpose (give the legitimate client a TCP fallback that spoofed-source floods cannot follow) is also moot on a stream transport, which already defeats source spoofing. Fix: on a stream transport the slip path now delivers the real answer instead of the TC=1 stub. Pin: [server/stream_no_truncate_test.go](server/stream_no_truncate_test.go) `TestRRLSlip_StreamDeliversRealAnswer`.

## [0.8.27] - 2026-05-29

### Fixed
- **Data race on `handleTimeSeriesWS` ticker init vs concurrent client subscription update** — the WebSocket time-series streamer at [web/timeseries_ws.go:178](web/timeseries_ws.go) was initialising its main-loop ticker by dereferencing the shared subscription pointer (`time.NewTicker(sub.PushEvery)`) while the read goroutine spawned six lines earlier was concurrently rewriting `*sub = *newSub` whenever the client posted an update frame. The init at line 170-172 (`subMu.Lock(); initialSub := *sub; subMu.Unlock()`) correctly copied the snapshot for the first push, but the very next line read `sub.PushEvery` directly off the still-shared pointer. Same shape as the v0.8.16 family of publication-race fixes, narrower scope: a single field read on the WS hot path. Two failure modes: (1) the Go race detector flags it on every concurrent CI run that exercises live subscription updates — a recurring red signal that buries other findings; (2) on a weak-ordering CPU the ticker could pick up a torn `time.Duration` read mid-rewrite (int64 reads are not necessarily atomic without sync) and either schedule with a nonsense interval (sub-millisecond or hours-long) or panic in `time.NewTicker` on a non-positive value, killing the WS handler goroutine and leaving the client without snapshots. The `lastPush` variable a few lines down was also initialised to the zero value, so the first tick's `if currentSub.PushEvery != lastPush` comparison always fired a redundant `ticker.Reset` to the same interval — minor inefficiency adjacent to the bug. Fix uses the already-existing `initialSub` snapshot (copied under `subMu` at line 170-172) for both the ticker init and the `lastPush` seed. The shared `*sub` is now only ever read under `subMu` from the main loop. Pin in [web/timeseries_ws_sub_race_test.go](web/timeseries_ws_sub_race_test.go): opens a WS connection, drains the initial snapshot to ensure the main loop reaches the ticker line, then blasts ~500ms of client subscription updates at the read goroutine; under `go test -race` this passes silently after the fix and flags the pre-fix code. The two other WS handlers (`handleQueryStreamWS`, the auth queries fanout) don't have this pattern because they don't accept client subscription updates — their `sub` analogue is fixed for the lifetime of the connection.

## [0.8.26] - 2026-05-29

### Fixed
- **`AdminServer.s.config` migrated to `atomic.Pointer[config.Config]`** — full publication of the rotating *config.Config across all 60+ reader and 4 writer sites in the web package, the last large unsynchronised admin-state surface flagged earlier in this stream. Before this release `s.config` was a plain `*config.Config` pointer rotated by `/api/config/raw` PUT (`s.config = parsedCfg` at api_config.go) and mutated in place by `/api/auth/change-password` (`s.config.Web.Auth.PasswordHash = newHash` at auth.go) under `configFileMu`, while every read site (admin GET handlers across api_cache.go, api_security.go, api_system_profile.go, api_tls.go, api_top.go, api_update.go, middleware.go, server.go, plus the auth credential read in handleLogin) did `s.config.Foo` directly without sync. The on-disk lost-update gate from v0.7.58 (`TestChangePassword_NoLostUpdateVsConfigRaw`) and the targeted reader-side `configFileMu` extension from v0.8.24 (`TestHandleLogin_CredentialReadIsRaceSafeVsPasswordChange`) closed the security-critical sub-races, but `go test -race` still flagged data races on every other reader vs writer interleaving, and on a sufficiently weak-ordering CPU a half-applied snapshot could in principle be observed for fields like `Web.TLSEnabled` or `Server.DoTEnabled` that gate connection-time branches. The migration: (a) field changed from `*config.Config` to `atomic.Pointer[config.Config]`; (b) `NewAdminServer` does `s.config.Store(cfg)` after the struct literal; (c) the two wholesale-swap writers (`api_config.go:367`, `api_dashboard_layout.go:149`) now `Store`; (d) the two in-place mutators (`auth.go` password change, `api_dashboard_layout.go` parse-fallback) now copy-on-write — Load → shallow struct copy → mutate the copy's field → Store — which is correct because the mutated fields are Go strings or fresh caller-allocated slices (no aliasing back into the previous snapshot readers are still holding); (e) the v0.8.24 reader-side `configFileMu` extension on `handleLogin` is retired in favour of the proper atomic publication (the reader no longer pays the lock cost). `configFileMu` is retained on writers for the on-disk lost-update gate. Production reader sites now uniformly use `s.config.Load().Foo`; tests use the same pattern (single-threaded test contexts where in-place mutation on the loaded snapshot is safe). Pin coverage continues via the two existing tests, both still passing.
...
### Fixed
- **Data race on `handleLogin` credential read vs `handleChangePassword` rotation** — `handleLogin` did `cfgUser := s.config.Web.Auth.Username` followed by `cfgHash := s.config.Web.Auth.PasswordHash` without holding any mutex, while `handleChangePassword` rewrites `s.config.Web.Auth.PasswordHash = newHash` under `configFileMu` (and `handleConfigRaw` PUT swaps `s.config = parsedCfg` under the same lock). The on-disk lost-update race was already pinned by v0.7.58 / `TestChangePassword_NoLostUpdateVsConfigRaw`, but the IN-MEMORY race on the same fields was wide open: every login attempt that landed while the change-password handler was mid-rewrite read the Go string header (16-byte ptr+len struct) without synchronisation. Two failure modes: (1) the Go race detector flags it on every CI run that exercises concurrent login + rotation paths — a recurring red signal that buried other findings; (2) on a sufficiently adversarial scheduler the login handler could in principle observe a torn header — old pointer with new length, or new pointer with old length — and bcrypt against the resulting garbage bytes would reject the operator's correct password intermittently during the rotation window. Smallest exposure window in production (single struct-field rewrite is sub-microsecond) but maximally visible in race-detector CI. Fix extends `configFileMu` coverage to the reader: the login handler now takes the lock briefly around the two field copies and releases before the bcrypt verify (so the expensive verify still runs lock-free). The mutex is the same one the writers hold; lock contention is bounded by the rate-limited login QPS and the rare admin rotation. Pin in [web/auth_login_credential_race_test.go](web/auth_login_credential_race_test.go): 16 readers × 50 login attempts in parallel with a writer rotating the password back and forth — the test passes silently under `go test`, but `go test -race` will report any future regression that drops the lock coverage. Other call sites (`ensurePasswordHashUnchanged` at api_config.go:267, `handleChangePassword` at auth.go:324) already read under `configFileMu` because they're nested inside the writer's lock window; this fix closes the only remaining unsynchronised reader of the auth fields.
